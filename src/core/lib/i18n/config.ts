...
// `isDirectionRestartPending()` used to live here. It is gone deliberately:
// reading it during render is the imperative pattern story 25 removed — it
// re-rendered only when the component happened to call `t()` as well. Use
// `useLocale().restartPending`, which subscribes.
